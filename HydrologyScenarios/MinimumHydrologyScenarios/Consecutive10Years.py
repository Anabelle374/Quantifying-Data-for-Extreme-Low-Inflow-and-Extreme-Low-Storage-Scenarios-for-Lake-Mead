...
# Output
output_file = f"{year}ConsecutivefailHydrologyResults.xlsx" # Names output a unique name
...
for ensemble_input in sheet_names:  # Iterates through ensembles
    ...
    if 'ISM' in ensemble_input: # Selects ensembles with 'ISM' in the title
        ...
        # Searches for ALL qualifying sums of consecutive years
        for ism_trace in ensemble.columns[1:2]:  # Iterates through the first trace in each ISM ensemble
            ...
            qualifying_windows = valid[valid < threshold]

            for end_idx in qualifying_windows.index:
                start_idx = end_idx - (year - 1)
...
df = df.sort_values(by=['Sum', 'Average'], ascending=True).reset_index(drop=True) # Sorts results
...
print("\nThe code iterated through", count_ensembles, "ensembles and", sum(all_traces), "windows.") # Displays number of iterations
